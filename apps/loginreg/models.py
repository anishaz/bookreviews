...
class UserManager(models.Manager):
    ...
    def registration(self, data):
        ...
        for field in data:
            if len(data[field]) == 0:
                errors.append("No field must be left blank. Please check and re-submit.")
        ...
        if len(data['first_name']) > 2 and not NAME_REGEX.match(data['first_name']):
            errors.append("Invalid First Name. Name must only contain letters.")

        if len(data['last_name']) > 2 and not NAME_REGEX.match(data['last_name']):
